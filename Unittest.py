...
def run_all_cases_test(X_train, y_train, X_test, y_test,
    ...
    for opt in optimizers:
        ...
        for pen in penalties:
            ...
            if opt == "gd":
                w,losses = gradient_descent_run_all(X_train, y_train, X_test, y_test,learning_rates, epochs=200, penalty=pen, lam=0.01)
            elif opt == "backtracking":
                w, losses = backtracking_gd(X_train, y_train, X_test, y_test, alpha_0, epochs=epochs, penalty=pen, lam=lam)
            elif opt == "minibatch":
                w, losses = mini_batch_gd(X_train, y_train, X_test, y_test, learning_rates, epochs=1000, batch_size=32, penalty=pen, lam=0.01)
            elif opt == "sgd":
                ...
                w, losses = accelerated_gd_run_all(X_train, y_train, X_test, y_test,learning_rates, epochs=200, penalty=pen, lam=0.01)
    return results
